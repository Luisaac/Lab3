...
public class Lab3 {
		...
		for (Instance image : dataset.getImages()) {
			...
			for(int i = 0; i<len;i++){
				for(int j = 0;j<len;j++){
					...
					//green[i][j] = image.getGrayChannel()[i][j]/255.0;
				}
			}
			result.add(red);
			result.add(blue);
	...
	public static void backward(Layer C1_layer, Layer C2_layer, Vector<double[][]> rhs, Vector<double[][]> v){
		...
		for(int i = 0; i < C2_layer.plates.length; i++){
			System.out.println("\n"+i+" layer count:");
			int count = 0;
			double [][] local_delta = new double [c2_maxtrix1_len][c2_maxtrix1_len];
						...
						for(int ki = 0; ki < C1_layer.kernal_length; ki++){
							for(int kj = 0; kj < C1_layer.kernal_length; kj++){
								...
								if((j-ki) <0 || (k-kj) <0 || (j-ki) >= deltalen || (k-kj) >= deltalen)
									...
									mhs_matrix[j][k] +=  deltas_2.get(deltaIndex)[j-ki][k-kj] * C1_layer.kernals.get(i)[ki][kj];
								}
							}
						}
					}
		...
		//System.out.println("first layer weights delta: ");
		// update weight 1
		for(int i = 0; i < deltas_2.size(); i++){
			...
			for(int ki = 0; ki < C2_layer.kernal_length; ki++){
				for(int kj = 0; kj < C2_layer.kernal_length; kj++){
					...
					// update each weight by multiplying all inputs in the previous layer* delta
					double kernal_delta = 0;
					...
					//System.out.println(C2_layer.plates[i].kernal[ki][kj]);

					C2_layer.plates[i].kernal[ki][kj] -= learningRate*kernal_delta;
				}
			}
			// update bias delta
			for(int ai = 0; ai < c2_maxtrix1_len; ai++){
					...
					for(int j = 0; j < v.size(); j++){
						for(int ai = 0; ai < c1_maxtrix1_len; ai++){
							for(int aj = 0; aj < c1_maxtrix1_len; aj++){
								kernal_delta += deltas_1.get(i)[ai][aj]*v.get(j)[ai+ki][aj+kj];
							}
						}
					}
