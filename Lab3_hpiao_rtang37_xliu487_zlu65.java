...
public class Lab3_hpiao_rtang37_xliu487_zlu65 {
	...
	static FCLayer hiddenLayer = new FCLayer(numHU,numLinks+1, true);
...
class Neuron {
	...
	public Neuron(int numLinks, boolean sigmoid){
		...
		for(int i = 0; i<numLinks;i++){
			...
			weights[i] = -0.3+rand.nextDouble()*0.6;
		}
		bias = -1;
	}
...
class Layer{
	...
	public Layer(int num_input_plate, int num_plate, int kernal_length, int pooling_length, int input_size){
			...
			//	System.out.println("left " + a + "right" + index);
				...
				for(int i = 0; i < kernal_length; i++){
					for(int j = 0; j < kernal_length; j++){		
						kernal[a][i][j] = getRandom(5*5*num_input_plate+1,20*25);
		...
		for(int i = 0; i < biasWeight.length; i++){
			biasWeight[i] = getRandom(5*5*num_input_plate+1,20*25);
		}
...
//		System.out.print("i = "+i_max);
//		System.out.print(" j = "+j_max);
